...
from lasagne.nonlinearities import softmax, sigmoid, linear, rectify
...
def get_model(input_var, target_var, multiply_var):
    ...
    layer_1         = batch_norm(Conv3DDNNLayer(incoming=layer_0, num_filters=16, filter_size=(1,3,3), stride=(1,1,1), pad='same', nonlinearity=rectify))
    layer_2         = batch_norm(Conv3DDNNLayer(incoming=layer_1, num_filters=16, filter_size=(3,1,1), stride=(1,1,1), pad='same', nonlinearity=rectify))
    layer_3         = MaxPool3DDNNLayer(layer_2, pool_size=(1, 2, 2), stride=(1, 2, 2), pad=(0, 1, 1))
    layer_4         = DropoutLayer(layer_3, p=0.25)

    # Convolution then batchNormalisation then activation layer, then zero padding layer followed by a dropout layer
    layer_5         = batch_norm(Conv3DDNNLayer(incoming=layer_4, num_filters=32, filter_size=(1,3,3), stride=(1,1,1), pad='same', nonlinearity=rectify))
    layer_6         = batch_norm(Conv3DDNNLayer(incoming=layer_5, num_filters=32, filter_size=(3,1,1), stride=(1,1,1), pad='same', nonlinearity=rectify))
    layer_7         = MaxPool3DDNNLayer(layer_6, pool_size=(1, 2, 2), stride=(1, 2, 2), pad=(0, 1, 1))
    layer_8         = DropoutLayer(layer_7, p=0.25)
    
    # Convolution then batchNormalisation then activation layer, then zero padding layer followed by a dropout layer
    layer_5         = batch_norm(Conv3DDNNLayer(incoming=layer_8, num_filters=64, filter_size=(1,3,3), stride=(1,1,1), pad='same', nonlinearity=rectify))
    layer_6         = batch_norm(Conv3DDNNLayer(incoming=layer_5, num_filters=64, filter_size=(3,1,1), stride=(1,1,1), pad='same', nonlinearity=rectify))
    layer_7         = batch_norm(Conv3DDNNLayer(incoming=layer_6, num_filters=64, filter_size=(3,3,3), stride=(1,1,1), pad='same', nonlinearity=rectify))
